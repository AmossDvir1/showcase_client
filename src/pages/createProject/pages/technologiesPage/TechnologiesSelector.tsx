...
export const TechnologiesSelector: React.FC<Props> = ({
  ...
  useEffect(() => {
    const colorSets = items.map((item, index: number) => {
      ...
      return {
        value: item,
        color: getAvarageRGBValue(color) > 127 ? "rgb(0,0,0)" : "rgb(255,255,255)",
        bgColor: color,
      };
    });
  ...
  const onUpdateGenres = (chips: string[]) => {
    ...
    const chipsWithColors = chips.map((chip) => availableGenres.find((colorSet) => colorSet.value === chip));
        ...
        >
          {items.map((item: string | number) => (
            <MenuItem
              key={item}
              value={item}
              //   style={getStyles(item, chipsPicked, theme)}
            >
              {item}
            </MenuItem>
          ))}
