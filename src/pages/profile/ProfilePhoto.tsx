...
const ProfilePhoto: React.FC<ProfilePhotoProps> = ({ userProfile = false, profilePicture=null }) => {
  ...
  return (
    <div className="rounded-full border-solid border-white border-4">
      <ProfilePictureUploader setOpen={setUploaderOpen} open={uploaderOpen}></ProfilePictureUploader>
      {isHovered && userProfile && !profilePicture ? (
        ...
        <PersonIcon
          ...
          onMouseLeave={() => setIsHovered(false)}
        ></PersonIcon>
      ):<img onClick={() => userProfile && setUploaderOpen(true)} className="rounded-full w-40 h-40 lg:w-40 lg:h-40 xs:w-[6rem] xs:h-[6rem] hover:brightness-90" src={profilePicture} alt="profilePicture"></img>}
    </div>
  );
};
